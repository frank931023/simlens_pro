...
本文檔補充 SimLens 系統中關於 persona (虛擬觀眾) 量化和評估的研究基礎。我們探討如何將個性、工作、興趣、MBTI、Big Five 等人格特質量化為可測量的指標,並提出客觀評估虛擬觀眾行為準確性的方法。
...
- 高分特徵: 好奇、創造力強、喜歡探索
- 低分特徵: 保守、傳統、偏好熟悉事物
- 與影片偏好關聯: 高開放性觀眾偏好多樣化、創新內容

**2. Conscientiousness (盡責性)**: 組織性和目標導向程度
- 測量範圍: 0-100 分
- 高分特徵: 有條理、負責任、自律
- 低分特徵: 隨性、靈活、自發
- 與影片偏好關聯: 高盡責性觀眾偏好教育性、結構化內容

**3. Extraversion (外向性)**: 社交活躍度和能量來源
- 測量範圍: 0-100 分
- 高分特徵: 外向、健談、尋求刺激
- 低分特徵: 內向、安靜、偏好獨處
- 與影片偏好關聯: 高外向性觀眾偏好社交、娛樂性內容

**4. Agreeableness (親和性)**: 合作和同理心程度
- 測量範圍: 0-100 分
- 高分特徵: 友善、信任他人、樂於助人
- 低分特徵: 競爭性強、懷疑、直接
- 與影片偏好關聯: 高親和性觀眾偏好溫馨、正面情感內容

**5. Neuroticism (神經質)**: 情緒穩定性
- 測量範圍: 0-100 分
- 高分特徵: 焦慮、情緒波動、敏感
- 低分特徵: 冷靜、情緒穩定、抗壓
- 與影片偏好關聯: 高神經質觀眾可能避免高壓力、緊張內容
...
**3. Diversity (多樣性)**
```
T_diversity(u) = |∪ G_i| (觀眾觀看的影片類型總數)
```
- 測量: 觀眾觀看內容的類型廣度
- 分層: 低多樣性 (專注特定類型), 中多樣性, 高多樣性 (廣泛興趣)
- 影響: 高多樣性觀眾對不同風格內容接受度更高
...
**Age → 影片特徵權重**
- 年輕觀眾 (18-24) → 高 SCR 和 OFM 權重 (偏好快節奏、動態內容)
- 中年觀眾 (35-50) → 高 SD 權重 (偏好信息密集內容)
- 老年觀眾 (55+) → 低 SCR 權重 (偏好慢節奏內容)
...
基於 arXiv:2407.18416v4 的 PersonaGym 框架,評估 persona 虛擬觀眾的一致性。

**PersonaScore 指標**

PersonaScore 基於決策理論,評估虛擬觀眾行為與 persona 描述的一致性:
...
測試虛擬觀眾是否能準確識別符合其 persona 的影片:

```python
# 設置: 給虛擬觀眾 20 個影片,其中 m 個是符合其偏好的
# 評估: 虛擬觀眾能否正確識別這些影片
...
測試虛擬觀眾的評分分佈是否符合真實觀眾模式:

```python
def evaluate_rating_distribution(agent_ratings, real_audience_ratings):
    """
    比較虛擬觀眾評分分佈與真實觀眾評分分佈
    """
    # KL Divergence: 越小越好
...
測試虛擬觀眾的社交特質 (activity, conformity, diversity) 是否符合設定:

```python
def evaluate_social_traits(agent, expected_traits):
    """
    測量虛擬觀眾實際表現的社交特質與預期的差異
    """
...
```python
# 模擬 A/B 測試,比較虛擬觀眾反饋與真實觀眾反饋的一致性

def simulate_ab_test(prediction_algorithm_A, prediction_algorithm_B):
    # 使用虛擬觀眾評估兩個預測算法
...
**評估指標**:
- Agreement Rate: 虛擬觀眾與真實觀眾得出相同結論的比例
- 目標: > 80% agreement rate

**方法 3: Causal Discovery Validation**

基於 Agent4Rec 的因果發現方法,驗證虛擬觀眾是否能揭示真實的因果關係:
...
**評估指標**:
- Edge Precision: 虛擬觀眾發現的因果關係中有多少是真實存在的
- Edge Recall: 真實存在的因果關係中有多少被虛擬觀眾發現
...
**方法**:
1. 從 MicroLens-100K 學習觀眾權重
2. 為每個觀眾標註 persona 特質 (基於觀看歷史)
...
**目的**: 驗證虛擬觀眾行為與 persona 描述的一致性

**方法**:
1. 創建 1,000 個虛擬觀眾,每個有不同的 persona
2. 讓虛擬觀眾模擬觀看行為 (觀看、評分、退出)
...
**目的**: 評估虛擬觀眾反饋與真實觀眾反饋的差距

**方法**:
1. Holdout Validation: 在測試集上比較虛擬觀眾預測與真實行為
2. A/B Testing Simulation: 比較虛擬觀眾和真實觀眾對預測算法的偏好
3. Causal Discovery: 比較虛擬觀眾和真實觀眾揭示的因果關係
